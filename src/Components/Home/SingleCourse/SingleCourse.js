...
const SingleCourse = () => {
  ...
  React.useEffect(() => {
    axios
      .get(`https://research-backend-production.up.railway.app/courses/${id}`)
      .then((res) => {
  ...
  React.useEffect(() => {
    axios
      .get(`https://research-backend-production.up.railway.app/promo`)
      .then((res) => {
        setPromos(res.data);
      });
  }, []);
  ...
  const handlePay = () => {
    ...
    fetch(`https://research-backend-production.up.railway.app/init`, {
      method: "POST",
      headers: {
            ...
            {user?.email ? (
              ...
              <Link to="/login" style={{ textDecoration: "none" }}>
                <button
                  ...
                  type="submit"
                >
                  Login To Get This Course at ${fee || sigleData?.courseFee}{" "}
                  &nbsp;&nbsp;
                  <FontAwesomeIcon
                    className="text-white pr-2 text-xl"
                    icon={faArrowRight}
