...
const NeedSupportDetailsModal = ({ handleSolveId, setHandleSolveModal }) => {
  ...
  React.useEffect(() => {
    axios
      .get(
        `https://research-backend-production.up.railway.app/singlesupportsession/${handleSolveId}`
      )
      .then((res) => {
  ...
  const handleStatus = (datas, status, mark) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put(
            `https://research-backend-production.up.railway.app/singlesupportsessionstatus/${handleSolveId}`,
            {
              data,
            }
          )
