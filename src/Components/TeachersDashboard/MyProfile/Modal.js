...
const Modal = ({ setShowModal }) => {
  ...
  React.useEffect(() => {
    axios
      .get(
        `https://research-backend-production.up.railway.app/singleTeacher?email=${user?.email}`
      )
      .then((res) => {
        reset(res?.data);
        setDetails(res?.data);
  ...
  const onSubmit = (data) => {
    ...
    axios
      .put(
        `https://research-backend-production.up.railway.app/teacherAbout?email=${user?.email}`,
        datas
      )
