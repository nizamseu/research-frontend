...
const PermanentAdd = ({ setPermanentAddModal }) => {
  ...
  React.useEffect(() => {
    axios
      .get(
        `https://research-backend-production.up.railway.app/allusers?email=${user?.email}`
      )
      .then((res) => {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .put(
        `https://research-backend-production.up.railway.app/permanentaddress?email=${user?.email}`,
        data
      )
